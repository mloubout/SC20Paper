...
We subsequently managed to work around point one above. Initial scaling experiments have been published in [..., page ..., figure ...], although this work still doesn't exploit the latest advancements in Devito v4.2 (e.g., computation/communication overlap). Our manuscript centers on the development of HPC production-grade wave propagators from a high-level language, and while performance is one of the cornerstones of our work, showing off Devito at extreme scale was not an objective. We have preliminary evidence that the latest Devito v4.2 has remarkable parallel efficiency, but we felt that publishing these results today would be premature -- we at least need testing and reproducibility on several architectures and applications.
