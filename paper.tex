...
finite-difference method. \textcolor{red}{The language is by design flexible enough to
express other non finite-difference operators, such as
interpolation and tensor contractions, inherent in
...
notoriously diverse and often complicated. The \textcolor{red}{DSL} is, however,
...
\begin{equation}
\begin{cases}
 m(x) \frac{\partial^2 u(t, x)}{\partial t^2} - \Delta u(t, x) = \delta(x_s) q(t) \\
...
\end{equation}
%
\textcolor{red}{where $u(t, x)$ is the pressure wavefield, $x_s$ is the point source position, 
$q(t)$ is the source time signature, $d(t, .)$ is the measured data at positions $x_r$ and $m(x)$ is the
squared slowness.}
...
\subsection{3D Imaging on Azure}\label{d-imaging-example-on-azure}
...
fully automatized staggered grid and staggered finite-differences which are generally required for a stable discretization
...
and consistent manner, the expression of all-kinds of non-trivial differential operators\textcolor{red}{: the vectorial elastic equation 
and TTI (rotated finite-difference) being two examples presented in the present study}. Next, and most
...
\textcolor{red}{We emphasize that the work
presented here does not solve (and does not claim to solve) all problems encountered within exploration geophysics. The high-level interface 
and at (industry) scale performance demonstrated has resulted in
multiple academic and industrial projects adopting the use \devito within their larger imaging frameworks.}
...
\begin{itemize}
  ...
  the TTI propagators, the Azure setup for shot parallelism and documentation.
  \item \textcolor{red}{Elastic simulations can be run using the elastic example} available in Devito at
  \url{https://github.com/devitocodes/devito/blob/v4.2/examples/seismic/elastic/elastic_example.py}. A standalone script to run the large 3D elastic modelling \textcolor{red}{experiment presented here}
